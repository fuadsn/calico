...
object PoseAngles {
    ...
    fun select(e: Exercise, xyz: FloatArray, visibility: FloatArray): Sample? {
        ...
        // Torso vector mid-hip -> mid-shoulder. Upright = shoulders clearly above hips AND not much
        // closer to the camera: a front-view pushup looks upright in 2D but the shoulders sit > 0.45
        // nearer in model depth, while standing stays within ±0.15.
        val dx = (xyz[11 * 3] + xyz[12 * 3] - xyz[23 * 3] - xyz[24 * 3]) / 2f
        val dy = (xyz[11 * 3 + 1] + xyz[12 * 3 + 1] - xyz[23 * 3 + 1] - xyz[24 * 3 + 1]) / 2f
        val dz = (xyz[11 * 3 + 2] + xyz[12 * 3 + 2] - xyz[23 * 3 + 2] - xyz[24 * 3 + 2]) / 2f
        val upright = dy < 0 && abs(dy) > abs(dx) && dz > -0.35f
        if (e.orientation == Orientation.UPRIGHT && !upright || e.orientation == Orientation.HORIZONTAL && upright) return null
