...
data class CoachState(val ready: Boolean, val busy: Boolean = false, val progress: Float? = null,
    ...
    val snapshot: CoachSnapshot? = null, val completedAnswer: String? = null)
...
class CoachViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val mutable = MutableStateFlow(CoachState(model.present(), snapshot=store.read()))
    val state = mutable.asStateFlow()

    fun refresh() { mutable.update { it.copy(ready=model.present(),snapshot=store.read()) } }
    ...
    fun send(question: String) {
        ...
        val prompt = try { CoachKnowledge.prompt(text,current.messages,current.snapshot) }
        ...
        task = viewModelScope.launch {
            try {
                withContext(dispatcher) {
                    ...
                    ensureActive()
                    mutable.update { it.copy(completedAnswer=CoachReplyPolicy.visible(output.toString("UTF-8")),
                        messages=it.messages.dropLast(1)+CoachMessage(false,CoachReplyPolicy.visible(output.toString("UTF-8")))) }
                }
            } catch (_: CancellationException) {
    ...
    fun forgetWorkout() { store.clear(); refresh() }
    fun pause() {
        stop()
